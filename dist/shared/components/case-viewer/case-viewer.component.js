...
var activity_2 = require("../../services/activity");
...
var case_view_1 = require("../../domain/case-view");
var dialogs_1 = require("../../components/dialogs");
var alert_1 = require("../../services/alert");
var draft_1 = require("../../services/draft");
...
var CaseViewerComponent = /** @class */ (function () {
    function CaseViewerComponent(ngZone, route, navigationNotifierService, orderService, activityPollingService, dialog, alertService, draftService, caseNotifier, errorNotifierService, location) {
        ...
        this.BANNER = activity_1.DisplayMode.BANNER;
    ...
    CaseViewerComponent.prototype.applyTrigger = function (trigger) {
        ...
        if (trigger.id === case_view_1.CaseViewTrigger.DELETE) {
            ...
            dialogRef.afterClosed().subscribe(function (result) {
                if (result === 'Delete') {
                    _this.draftService.deleteDraft(_this.caseDetails.case_id)
                        ...
                        _this.navigationNotifierService.announceNavigation({ action: navigation_1.NavigationOrigin.DRAFT_DELETED });
                    }, function (_) {
                        _this.navigationNotifierService.announceNavigation({ action: navigation_1.NavigationOrigin.ERROR_DELETING_DRAFT });
                    });
                }
            });
        }
        else if (this.isDraft() && trigger.id !== case_view_1.CaseViewTrigger.DELETE) {
            ...
            this.navigationNotifierService.announceNavigation({
                action: navigation_1.NavigationOrigin.DRAFT_RESUMED,
            ...
            this.navigationNotifierService.announceNavigation({
                action: navigation_1.NavigationOrigin.EVENT_TRIGGERED,
                queryParams: theQueryParams,
                etid: trigger.id,
    ...
    CaseViewerComponent.prototype.getTabFields = function () {
        var caseDataFields = this.sortedTabs.reduce(function (acc, tab) {
            return acc.concat(class_transformer_1.plainToClass(definition_1.CaseField, tab.fields));
        }, []);
    ...
    CaseViewerComponent = CaseViewerComponent_1 = __decorate([
        ...
        __metadata("design:paramtypes", [core_1.NgZone,
            ...
            navigation_1.NavigationNotifierService,
            order_1.OrderService,
            activity_2.ActivityPollingService,
            material_1.MatDialog,
            alert_1.AlertService,
            draft_1.DraftService,
            case_editor_1.CaseNotifier,
            error_1.ErrorNotifierService,
            common_1.Location])
    ], CaseViewerComponent);
