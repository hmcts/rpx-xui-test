...
var FormValueService = /** @class */ (function () {
    ...
    FormValueService.getFieldValue = function (form, fieldKey, colIndex) {
        ...
        if (fields_1.FieldsUtils.isMultiSelectValue(currentForm)) {
            return form[currentFieldId + FormValueService_1.LABEL_SUFFIX].join(', ');
        }
    ...
    FormValueService.prototype.sanitiseObject = function (rawObject) {
        ...
        Object.keys(rawObject).forEach(function (key) {
            if ('CaseReference' === key) {
                sanitisedObject[key] = _this.sanitiseValue(_this.sanitiseCaseReference(String(rawObject[key])));
            }
            else {
                ...
                if (field.hidden === true && field.display_context !== 'HIDDEN') {
                    ...
                    switch (field.field_type.type) {
                        ...
                        case 'Complex':
                            ...
                            if (FormValueService_1.clearOptionalEmpty(clearEmpty, data[field.id], field)) {
                                delete data[field.id];
                            }
                            break;
                        case 'Collection':
    ...
    FormValueService.LABEL_SUFFIX = '-LABEL';
